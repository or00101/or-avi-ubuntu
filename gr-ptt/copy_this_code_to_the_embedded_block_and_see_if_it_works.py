...
class blk(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
	...
	def __init__(self):  # only default arguments here
		...
		self.amp_driver = Amp_Driver(13)
		

	def work(self, input_items, output_items):
		if self.is_input_all_zeros(input_items):
			self.amp_driver.shut_down()
			output_items[0][:] = 1 # When debugging this will allow us to plug this block's output  to a time sink and see if and when the driver woke up
		else:
			self.amp_driver.wake_up()
			output_items[0][:] = 0 # When debugging this will allow us to plug this block's output to a time sink and see if and when the driver shut down
